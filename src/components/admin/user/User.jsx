import { useQuery } from '@tanstack/react-query'
import dayjs from 'dayjs'
import React, { useState } from 'react'
import { apiUrl } from '../../common/Http'
import Layout from '../../common/Layout'
import Loader from '../../common/Loader'
import MyModal from '../../common/myModel'
import Sidebar from '../../common/Sidebar'

const User = () => {
    const [showModal, setShowModal] = useState(false);
    const [selectedUser, setSelectedUser] = useState(null);
    const handleShowUser = (user) => {
        setSelectedUser(user);
        setShowModal(true); 
    };

    const handleCloseModal = () => {
        setShowModal(false);
        setSelectedUser(null);
    };
    const fetchUsers = async () => {
        const res = await fetch(`${apiUrl}/admin/users`, {
            method: 'GET',
            headers: {
                'Content-type': 'application/json',
                'Accept': 'application/json'
            },
            credentials: 'include'
        });

        const result = await res.json();
        if (result.status === 200) {
            return result.data;
        }
        return [];
    }

    const { data: users = [], isLoading: loadingUsers } = useQuery({
        queryKey: ['users'],
        queryFn: fetchUsers
    });
    return (
        <Layout>
            {
                showModal && <MyModal show={showModal}
                    onClose={handleCloseModal}
                    user={selectedUser}
                />
            }
            <div className='container products py-5'>
                <h3>Products</h3>
                <div className='row'>
                    <div className='col-lg-3 shadow rounded p-3 mb-4'>
                        <Sidebar />
                    </div>
                    <div className='col-lg-9'>
                        <div className='row'>
                            <div className='col-xl-12'>
                                <div className="card p-4 shadow">
                                    {
                                        loadingUsers ? <Loader /> :
                                            users && users.length > 0 &&

                                            <table className='table table-hover'>
                                                <thead>
                                                    <tr>
                                                        <th width={50}>ID</th>
                                                        <th>Name</th>
                                                        <th>Email</th>
                                                        <th>Date</th>
                                                        <th width={100}>Action</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    {
                                                        users.map((user, index) => {
                                                            return (
                                                                <tr key={index}>
                                                                    <td>{user.id}</td>
                                                                    <td>{user.name}</td>
                                                                    <td>{user.email}</td>
                                                                    <td>{dayjs(user.created_at).format("DD MMM YYYY , hh:mm A")}</td>
                                                                    <td>
                                                                        <svg onClick={() => handleShowUser(user)} style={{ cursor: 'pointer' }} className={'text-primary'} stroke="currentColor" fill="currentColor" strokeWidth="0" viewBox="0 0 512 512" height="1em" width="1em" xmlns="http://www.w3.org/2000/svg"><path d="M471.6 21.7c-21.9-21.9-57.3-21.9-79.2 0L362.3 51.7l97.9 97.9 30.1-30.1c21.9-21.9 21.9-57.3 0-79.2L471.6 21.7zm-299.2 220c-6.1 6.1-10.8 13.6-13.5 21.9l-29.6 88.8c-2.9 8.6-.6 18.1 5.8 24.6s15.9 8.7 24.6 5.8l88.8-29.6c8.2-2.7 15.7-7.4 21.9-13.5L437.7 172.3 339.7 74.3 172.4 241.7zM96 64C43 64 0 107 0 160V416c0 53 43 96 96 96H352c53 0 96-43 96-96V320c0-17.7-14.3-32-32-32s-32 14.3-32 32v96c0 17.7-14.3 32-32 32H96c-17.7 0-32-14.3-32-32V160c0-17.7 14.3-32 32-32h96c17.7 0 32-14.3 32-32s-14.3-32-32-32H96z"></path></svg>
                                                                    </td>
                                                                </tr>
                                                            );
                                                        })
                                                    }
                                                </tbody>
                                            </table>
                                    }
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </Layout>
    )
}

export default User